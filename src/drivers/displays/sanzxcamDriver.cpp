...
void sanzxcam_ClockScreen(unsigned long mElapsed)
{
...
void sanzxcam_GlobalHashScreen(unsigned long mElapsed)
{
...
void sanzxcam_BTCprice(unsigned long mElapsed)
{
  clock_data data = getClockData(mElapsed);
...
void sanzxcam_LoadingScreen(void)
{
  tft.fillScreen(TFT_BLACK);
  tft.pushImage(0, 35, initWidth, initHeight, initScreen);
  tft.setTextColor(TFT_BLACK);
  tft.drawString(CURRENT_VERSION, 24, 182, FONT2);
}

void sanzxcam_SetupScreen(void)
{
  tft.pushImage(0, 35, setupModeWidth, setupModeHeight, setupModeScreen);
}

void sanzxcam_AnimateCurrentScreen(unsigned long frame)
{
}

void sanzxcam_DoLedStuff(unsigned long frame)
{
}

CyclicScreenFunction sanzxcamCyclicScreens[] = {sanzxcam_MinerScreen, sanzxcam_ClockScreen, sanzxcam_GlobalHashScreen, sanzxcam_BTCprice};

DisplayDriver sanzxcamDisplayDriver = {
    sanzxcam_Init,
    sanzxcam_AlternateScreenState,
    sanzxcam_AlternateRotation,
    sanzxcam_LoadingScreen,
    sanzxcam_SetupScreen,
    sanzxcamCyclicScreens,
    sanzxcam_AnimateCurrentScreen,
    sanzxcam_DoLedStuff,
    SCREENS_ARRAY_SIZE(sanzxcamCyclicScreens),
    0,
    WIDTH,
    HEIGHT};
#endif
